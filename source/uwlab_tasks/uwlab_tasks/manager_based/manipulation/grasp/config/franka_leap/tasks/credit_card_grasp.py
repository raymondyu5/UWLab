...
# Note: GRASPED_Z=0.06, LIFTED_Z_LOW=0.10 match overrides in GraspCreditCardFrankaLeapCfg.__post_init__.
# ---------------------------------------------------------------------------

def _card_obs_object_pose(env) -> torch.Tensor:
    state = env.scene["grasp_object"]._data.root_state_w[:, :7].clone()
    state[:, :3] -= env.scene.env_origins
    return state


def _card_rew_grasped(env) -> torch.Tensor:
    pos = env.scene["grasp_object"]._data.root_state_w[:, :3] - env.scene.env_origins
    return (pos[:, 2] - CARD_SPAWN_POS[2] >= 0.06).float()


def _card_rew_lifted(env) -> torch.Tensor:
    pos = env.scene["grasp_object"]._data.root_state_w[:, :3] - env.scene.env_origins
    z_above = pos[:, 2] - CARD_SPAWN_POS[2]
    return ((z_above >= 0.10) & (z_above <= 0.50)).float()


def _card_rew_success(env) -> torch.Tensor:
    pos = env.scene["grasp_object"]._data.root_state_w[:, :3] - env.scene.env_origins
    target = torch.tensor(list(CARD_TARGET_POS), device=env.device, dtype=torch.float32)
    dist = torch.linalg.norm(pos - target.unsqueeze(0), dim=1)
    return (dist <= 0.15).float()


def _card_rew_joint_vel(env) -> torch.Tensor:
    return torch.sum(env.scene["robot"].data.joint_vel ** 2, dim=1)


def _card_rew_action_rate(env) -> torch.Tensor:
    return torch.sum((env.action_manager.action - env.action_manager.prev_action) ** 2, dim=1)


def _card_obs_target_pose(env) -> torch.Tensor:
    """Fixed target pose (7D): CARD_TARGET_POS xyz + object's default rotation."""
    target = torch.tensor([list(CARD_TARGET_POS)], device=env.device, dtype=torch.float32).expand(env.num_envs, -1)
...
def _card_obs_contact(env) -> torch.Tensor:
...
def _card_obs_object_in_tip(env) -> torch.Tensor:
...
@configclass
class GraspCreditCardFrankaLeapJointAbsStateCfg(GraspCreditCardFrankaLeapJointAbsCfg):
    ...
    def __post_init__(self):
        super().__post_init__()
        self.observations.policy.manipulated_object_pose = ObsTerm(func=_card_obs_object_pose)
        self.observations.policy.target_object_pose = ObsTerm(func=_card_obs_target_pose)
        self.observations.policy.contact_obs = ObsTerm(func=_card_obs_contact)
        self.observations.policy.object_in_tip = ObsTerm(func=_card_obs_object_in_tip)
        ...
        self.rewards.grasped = RewTerm(func=_card_rew_grasped, weight=1.0)
        self.rewards.lifted = RewTerm(func=_card_rew_lifted, weight=5.0)
        self.rewards.success = RewTerm(func=_card_rew_success, weight=10.0)
        self.rewards.wrist = None
        self.rewards.joint_vel = RewTerm(func=_card_rew_joint_vel, weight=-1e-3)
        self.rewards.action_rate = RewTerm(func=_card_rew_action_rate, weight=-0.1)
        self.events.capture_reset_height = None
        self.metrics_spec = {
            "is_success": _card_rew_success,
            "is_lifted": _card_rew_lifted,
            "is_grasped": _card_rew_grasped,
        }


@configclass
class GraspCreditCardFrankaLeapJointAbsStateCollectCfg(GraspCreditCardFrankaLeapJointAbsStateCfg):

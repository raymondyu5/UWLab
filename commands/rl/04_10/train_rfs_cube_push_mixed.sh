#!/bin/bash
#SBATCH --job-name=uwlab_rfs_cube_push_mixed
...
#SBATCH --output=logs/uwlab_rfs_cube_push_mixed_%j.out
#SBATCH --error=logs/uwlab_rfs_cube_push_mixed_%j.err
...
apptainer exec --nv \
  ...
  /bin/bash -c '
    ...
    ./uwlab.sh -p scripts/reinforcement_learning/sb3/rfs/train.py \
      --task UW-FrankaLeap-PushCubeToPoptart-JointAbs-v0 \
      --num_envs 1024 \
      --diffusion_path logs/bc_cfm_pcd_cube_push_mixed_0410_absjoint_h16_hist4_noextnoise \
      --cfg configs/rl/arm_rfs_joint_smooth_cfg.yaml \

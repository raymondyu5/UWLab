...
#SBATCH --output=logs/rfs_rs1_gpu_%j.out
#SBATCH --error=logs/rfs_rs1_gpu_%j.err
...
apptainer exec --nv \
  ...
  /bin/bash -c '
    ...
    ./uwlab.sh -p scripts/reinforcement_learning/sb3/rfs/train.py \
      ...
      --cfg configs/rl/rfs_cfg_absjoint_h16_rs1_gpu.yaml \
      --eval_spawn random_1_trial \
      --wandb_project rfs_uwlab \
      --wandb_run_name rfs_rs1_gpu \
      --asymmetric_ac \
      --enable_cameras \
      --headless
  '

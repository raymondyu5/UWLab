#!/bin/bash
#SBATCH --job-name=uwlab_rfs_cube_grasp_rr
...
#SBATCH --output=logs/uwlab_rfs_cube_grasp_rr_%j.out
#SBATCH --error=logs/uwlab_rfs_cube_grasp_rr_%j.err
...
apptainer exec --nv \
  ...
  /bin/bash -c '
    ...
    ./uwlab.sh -p scripts/reinforcement_learning/sb3/rfs/train.py \
      --task UW-FrankaLeap-GraspCubeRandomResets-JointAbs-v0 \
      --num_envs 1024 \
      --diffusion_path logs/bc_cfm_pcd_cube_grasp_0408_absjoint_h16_hist4_noextnoise \

#!/bin/bash
#SBATCH --job-name=uwlab_rfs_cup_pick
...
#SBATCH --output=logs/uwlab_rfs_cup_pick_%j.out
#SBATCH --error=logs/uwlab_rfs_cup_pick_%j.err
...
apptainer exec --nv \
  ...
  /bin/bash -c '
    ...
    ./uwlab.sh -p scripts/reinforcement_learning/sb3/rfs/train.py \
      ...
      --num_envs 1024 \
      --diffusion_path logs/bc_cfm_pcd_cup_pick_0331_absjoint_h16_hist4_noextnoise \

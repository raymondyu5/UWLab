#!/bin/bash
#SBATCH --job-name=rl_dsrl_asymac_v1
...
#SBATCH --output=logs/rl_dsrl_asymac_v1_%j.out
#SBATCH --error=logs/rl_dsrl_asymac_v1_%j.err

# DSRL training with asymmetric actor-critic.
# Actor sees abs ee_pose + hand_joint_pos only (real-world deployable).
# Critic sees full privileged sim state (object pose, contacts, etc.).

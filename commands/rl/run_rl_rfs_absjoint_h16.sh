#!/bin/bash
#SBATCH --job-name=rl_rfs_absjoint_h16
...
#SBATCH --output=logs/rl_rfs_absjoint_h16_%j.out
#SBATCH --error=logs/rl_rfs_absjoint_h16_%j.err

# RFS RL training: JointAbs action space, horizon=16, n_obs_steps=4.
# CFM base policy: absjoint_h16_hist4 checkpoint.
# PPO executes full 16-step chunk per step; asymmetric AC:
#   actor sees PCD embedding + 4-frame joint history + past actions (matches BC obs).
#   critic sees privileged sim state.
...
apptainer exec --nv \
  ...
  /bin/bash -c '
    ...
    ./uwlab.sh -p scripts/reinforcement_learning/sb3/rfs/train.py \
      ...
      --cfg configs/rl/rfs_cfg_absjoint_h16.yaml \

#!/bin/bash
#SBATCH --job-name=uwlab_rfs_cube_push_all_0416_fast_l40s_2gpu
...
#SBATCH --output=logs/uwlab_rfs_cube_push_all_0416_fast_l40s_2gpu_%j.out
#SBATCH --error=logs/uwlab_rfs_cube_push_all_0416_fast_l40s_2gpu_%j.err
...
apptainer exec --nv \
  ...
  --bind /gscratch/weirdlab/raymond/reset_poses_cube_pushing_bourbon_ep0_04_08.json:/workspace/uwlab/assets/reset_poses_cube_push.json \
  ...
  /bin/bash -c '
    ...
    ./uwlab.sh -p -m torch.distributed.run \
      ...
      --task UW-FrankaLeap-PushCubeToPoptartRandomResets-JointAbs-v0 \
      --num_envs 4096 \
      --diffusion_path logs/bc_cfm_pcd_cube_push_all_0416_absjoint_h16_hist4_noextnoise_fast \

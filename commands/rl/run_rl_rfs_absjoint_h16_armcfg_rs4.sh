#!/bin/bash
#SBATCH --job-name=rl_rfs_absjoint_h16_armcfg_rs4
...
#SBATCH --output=logs/rl_rfs_absjoint_h16_armcfg_rs4_%j.out
#SBATCH --error=logs/rl_rfs_absjoint_h16_armcfg_rs4_%j.err

# RFS RL training: JointAbs action space, horizon=16, n_obs_steps=4, residual_step=4.
# Executes 4 actions per PPO step from the 16-step diffusion chunk.
# CFM base policy: absjoint_h16_hist4 checkpoint.
# Uses arm_rfs_joint_cfg_rs4: full 23D residual, residual_step=4.
...
apptainer exec --nv \
  ...
  /bin/bash -c '
    ...
    ./uwlab.sh -p scripts/reinforcement_learning/sb3/rfs/train.py \
      ...
      --cfg configs/rl/arm_rfs_joint_cfg_rs4.yaml \

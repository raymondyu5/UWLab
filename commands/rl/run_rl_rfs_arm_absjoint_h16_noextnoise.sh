#!/bin/bash
#SBATCH --job-name=rfs_arm_noextnoise
...
#SBATCH --output=logs/rfs_arm_noextnoise_%j.out
#SBATCH --error=logs/rfs_arm_noextnoise_%j.err
...
apptainer exec --nv \
  ...
  /bin/bash -c '
    ...
    ./uwlab.sh -p scripts/reinforcement_learning/sb3/rfs/train.py \
      ...
      --num_envs 1024 \
      --diffusion_path logs/bc_cfm_pcd_bourbon_0325_absjoint_h16_hist4_noextnoise \

#!/bin/bash
#SBATCH --job-name=collect_distill_pour_0316
...
#SBATCH --output=logs/collect_distill_pour_0316_%j.out
#SBATCH --error=logs/collect_distill_pour_0316_%j.err

# Collect distillation trajectories for PourBottle using the asymmetric DSRL policy.
# Actor: PCD embedding (mesh_pc via frozen CFM PointNet) + ee_pose + hand_joint_pos.
# Saves: arm/hand joint pos, ee_pose, actions, noise predictions, seg_pc, rewards, dones.
# horizon=140 (trimmed from default 180), finger_smooth_alpha=0.7, clipping enabled.
...
/gscratch/weirdlab/raymond/commands/uwlab/run_in_container.sh '
  ./uwlab.sh -p scripts/imitation_learning/collect_distill_trajectories.py \
    ...
    --num_episodes 2000 \
    --num_warmup_steps 10 \
    --output_dir logs/distill_collection/pour_bottle_0316 \
    --headless
'

...
#SBATCH --mem=128G
#SBATCH --time=4:00:00
#SBATCH --output=logs/uwlab_eval_pour_dropout_%j.out
#SBATCH --error=logs/uwlab_eval_pour_dropout_%j.err
...
/mmfs1/gscratch/weirdlab/raymond/commands/uwlab/run_in_container.sh '
  ./uwlab.sh -p scripts/eval/play_bc.py \
    ...
    --sim_type distill \
    checkpoint=logs/bc_cfm_pcd_bourbon_0312_deltajoint_h16_hist4_dropout \
    action_horizon=1 \
    num_envs=4 \
    record_video=true \
